...
class SinglePeriod:
    def __init__(self, data):
        ...
        self.sampleSet = {}
        self.samplerArgs = {}
        self.sampler = LeapHybridCQMSampler(**self.samplerArgs)
        self.solution = {}
    ...
    def build_cqm():
        ...
        x = {stock: Integer("%s" %s, lower_bound = 0, upper_bound = data.maxShares[stock]) for stock in data.stockTickers}
    ...
    def solve_cqm():
        ...
        if not feasibleSamples:
            ...
            solution['returns'] = getReturns(solution['stocks'])
            solution['risk'] = getRisk(solution['stocks'])

            cost = sum([data.price[stock] * max(0, solution['stocks'][stock] - data.initialHoldings[stock]) for stock in data.stocks])
            sales = sum([data.pric[stock] * max(0, data.initialHoldings[stock] - solution['stocks'][stock]) for stock in data.stocks])
